    ...
    <>
      ...
      <nav className="flex-1 px-4 mt-4 space-y-1 overflow-y-auto">
        {navItems.map(item => {
          ...
          const isLiveMonitor = item.to === '/teacher/live-monitoring'
          return (
            <Link
              key={item.label}
              to={item.to}
            ...
            >
              ...
              {isLiveMonitor && liveCount != null && liveCount > 0 && (
                <span className="bg-green-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full animate-pulse">
                  {liveCount}
                </span>
              )}
              {active && !isLiveMonitor && <ChevronRight className="w-3 h-3 opacity-60" />}
            </Link>
          )
        })}
      </nav>
            ...
            <div className="flex items-center gap-3">
              {liveCount != null && liveCount > 0 && (
                <Link
                  ...
                  className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-green-600 text-white text-xs font-semibold rounded-xl hover:bg-green-700 transition-all"
                >
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 py-1 px-2 rounded-lg transition-colors ${
                active ? 'text-purple-600' : 'text-gray-400 hover:text-purple-600'
              }`}
            >
