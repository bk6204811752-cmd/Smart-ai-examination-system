...
class IntelligentQuestionRecommender {
  ...
  async recommend(request: RecommendationRequest): Promise<RecommendationResult> {
    const { studentProfile, targetSubject, numberOfQuestions, examPurpose } = request
    ...
    const selected = this.selectDiverseQuestions(
      scored,
      numberOfQuestions,
      request
    )
    ...
    if (constraints) {
      if (constraints.requiredTopics) {
        candidates = candidates.filter(q => 
          constraints.requiredTopics!.includes(q.topic)
        )
      }
      
      if (constraints.excludeTopics) {
        candidates = candidates.filter(q => 
          !constraints.excludeTopics!.includes(q.topic)
        )
      }
      
      if (constraints.questionTypes) {
        candidates = candidates.filter(q => 
          constraints.questionTypes!.includes(q.type)
        )
      }
      
      if (constraints.bloomsLevels) {
        candidates = candidates.filter(q => 
          constraints.bloomsLevels!.includes(q.blooms_taxonomy)
        )
      }
      ...
      if (constraints.minDifficulty) {
        ...
        candidates = candidates.filter(q => 
          this.difficultyToNumber(q.difficulty) >= minLevel
        )
      }
      ...
      if (constraints.maxDifficulty) {
        ...
        candidates = candidates.filter(q => 
          this.difficultyToNumber(q.difficulty) <= maxLevel
        )
      }
    }
  ...
  private calculateTopicRelevance(question: Question, profile: StudentProfile): number {
    ...
    const recentStudy = profile.studyPatterns.find(p => 
  ...
  private calculateLearningValue(question: Question, profile: StudentProfile): number {
    ...
    const masteredPrereqs = question.prerequisites.filter(p => 
      profile.masteredTopics.includes(p)
    )
    const prereqCompleteness = question.prerequisites.length > 0
      ? masteredPrereqs.length / question.prerequisites.length
      : 1
  ...
  private calculateOverallMetrics(
    ...
    const difficulties = recommendations.map(r => 
      this.difficultyToNumber(r.question.difficulty)
    )
    const averageDifficulty = difficulties.reduce((sum, d) => sum + d, 0) / difficulties.length * 100
    
    // Topic coverage
    const uniqueTopics = new Set(recommendations.map(r => r.question.topic))
    const topicCoverage = (uniqueTopics.size / Math.max(1, profile.weakTopics.length + profile.masteredTopics.length)) * 100
    
    // Learning progression (increasing difficulty)
    const progressionScore = this.calculateProgressionMetric(recommendations)
    
    // Estimated accuracy
    const estimatedAccuracy = recommendations.reduce((sum, r) => 
      sum + (100 - r.expectedDifficulty), 0
    ) / recommendations.length
    
    // Confidence (based on data availability)
    const avgUsage = recommendations.reduce((sum, r) => 
      sum + r.question.metadata.usageCount, 0
    ) / recommendations.length
  ...
  private estimateStudentLevel(profile: StudentProfile, topic: string): number {
    ...
    const avgCorrectDifficulty = correctDifficulties.reduce((sum, d) => sum + d, 0) / correctDifficulties.length
  ...
  private calculateProgressionMetric(recommendations: QuestionRecommendation[]): number {
    ...
    for (let i = 1; i < recommendations.length; i++) {
      ...
      if (currDiff >= prevDiff - 0.1) { // Allow slight decrease
        progressionCount++
      }
    }
