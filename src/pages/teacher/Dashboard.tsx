...
function StatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${config.cls}`}
    >
...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span
      className={`text-xs font-bold px-2 py-0.5 rounded-full border ${map[severity] || map.LOW}`}
    >
      {severity}
    </span>
  )
...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    ws.on('ai_violation', (data: any) => {
      ...
      addActivity({
        ...
        time: 'just now',
        icon: AlertTriangle,
        color: v.severity === 'CRITICAL' ? 'text-red-500' : 'text-amber-500',
        score: null,
      })
  ...
  const avgCompletion =
    myExams.length > 0
      ? myExams.reduce((s, e) => s + (e.completion_rate || 0), 0) / myExams.length
      : 0
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: Radio,
      color: '#10b981',
      bg: 'from-green-50 to-emerald-100/40',
    ...
    },
    {
      ...
      icon: Users,
      color: '#8b5cf6',
      bg: 'from-purple-50 to-violet-100/40',
    ...
    },
    {
      ...
      icon: AlertTriangle,
      color: '#f59e0b',
      bg: 'from-amber-50 to-yellow-100/40',
  ...
  const tools = [
    ...
    {
      title: 'Live Monitor',
      desc: 'Watch students in real-time',
      icon: Monitor,
      link: '/teacher/live-monitoring',
      from: 'from-green-500',
      to: 'to-emerald-500',
      hot: liveStudentCount > 0,
    },
    {
      title: 'AI Generator',
      desc: 'Auto-generate questions',
      icon: Brain,
      link: '/teacher/ai-generator',
        ...
        {liveAlerts.length > 0 && liveAlerts[0].severity === 'CRITICAL' && (
          ...
          >
            ...
            <p className="text-sm font-semibold flex-1">
              🚨 CRITICAL: Student <strong>{liveAlerts[0].student_id}</strong> —{' '}
              {liveAlerts[0].message}
            </p>
            <Link
              to="/teacher/live-monitoring"
              className="text-xs font-bold bg-white/20 hover:bg-white/30 px-3 py-1 rounded-lg transition-all shrink-0"
            >
              View Now →
            </Link>
          </motion.div>
          ...
          {statCards.map((card, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div
                  className={`w-9 h-9 ${card.iconBg} rounded-xl flex items-center justify-center shadow-sm`}
                >
            ...
            {tools.map((tool, i) => (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`w-10 h-10 bg-gradient-to-br ${tool.from} ${tool.to} rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}
                  >
                    ...
                    >
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-sm ${
                          exam.status === 'active'
                            ? 'bg-gradient-to-br from-green-500 to-emerald-500 shadow-green-200'
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                          ...
                          <Link
                            to={`/teacher/exam-results/${exam._id}`}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`flex items-center gap-1.5 text-xs font-semibold ${wsStatus === 'connected' ? 'text-green-600' : 'text-gray-400'}`}
                  >
                    <span
                      className={`w-2 h-2 rounded-full ${wsStatus === 'connected' ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`}
                    />
              ...
              {liveAlerts.length === 0 && recentActivity.length === 0 ? (
                <div className="py-10 text-center">
                  ...
                  <p className="text-xs text-gray-300 mt-1">
                    Violations will appear here in real-time
                  </p>
                </div>
              ) : (
                <div className="divide-y divide-gray-50 max-h-72 overflow-y-auto">
                  {liveAlerts.slice(0, 6).map(alert => (
                    ...
                    >
                      <AlertTriangle
                        className={`w-4 h-4 mt-0.5 shrink-0 ${alert.severity === 'CRITICAL' ? 'text-red-500' : alert.severity === 'HIGH' ? 'text-orange-500' : 'text-amber-400'}`}
                      />
                      <div className="flex-1 min-w-0">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: 'Students Enrolled', value: totalStudents || '—' },
                  {
                    ...
                    value: avgCompletion > 0 ? `${avgCompletion.toFixed(0)}%` : '—',
                  },
