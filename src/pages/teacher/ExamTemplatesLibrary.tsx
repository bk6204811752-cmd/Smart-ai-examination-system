...
export default function ExamTemplatesLibrary() {
  ...
  const applyTemplate = (t: ExamTemplate) =>
    navigate('/teacher/create-exam', { state: { template: t } })
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white/90">
                Pre-built exam templates to accelerate your assessment creation
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="text-2xl font-bold text-gray-900">
              {templates.filter(t => t.category === 'certification').length}
            </div>
              ...
              >
                ...
                <div className="p-6">
                  <div className="flex flex-wrap gap-2 mb-4">
                    <span
                      className={`text-xs px-2 py-1 rounded-full font-medium ${getCategoryColor(template.category)}`}
                    >
                      {template.category}
                    </span>
                    <span
                      className={`text-xs px-2 py-1 rounded-full font-medium ${getDifficultyColor(template.difficulty)}`}
                    >
                      {template.difficulty}
                    </span>
                  </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="font-medium capitalize">{previewTemplate.difficulty}</span>
                </div>
                <div>
                  <span className="text-gray-500">Duration:</span>{' '}
                  <span className="font-medium">{previewTemplate.duration} min</span>
                </div>
                <div>
                  <span className="text-gray-500">Questions:</span>{' '}
                  <span className="font-medium">{previewTemplate.totalQuestions}</span>
                </div>
                <div>
                  <span className="text-gray-500">Passing Score:</span>{' '}
                  <span className="font-medium">{previewTemplate.passingScore}%</span>
                </div>
                <div>
                  <span className="text-gray-500">Created By:</span>{' '}
                  <span className="font-medium">{previewTemplate.createdBy}</span>
                </div>
                <div>
                  <span className="text-gray-500">Usage:</span>{' '}
                  <span className="font-medium">{previewTemplate.usageCount} times</span>
                </div>
              </div>
              <div>
                ...
                <div className="space-y-1">
                  {previewTemplate.questions.map((q, idx) => (
                    <div key={idx} className="flex justify-between text-sm bg-gray-50 p-2 rounded">
                      <span>{q.type}</span>
                      <span>
                        {q.count} questions × {q.points} pts
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${previewTemplate.settings.randomizeQuestions ? 'bg-green-500' : 'bg-gray-300'}`}
                    />
                    Randomize Questions
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${previewTemplate.settings.randomizeOptions ? 'bg-green-500' : 'bg-gray-300'}`}
                    />
                    Randomize Options
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${previewTemplate.settings.showResults ? 'bg-green-500' : 'bg-gray-300'}`}
                    />
                    Show Results
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${previewTemplate.settings.allowReview ? 'bg-green-500' : 'bg-gray-300'}`}
                    />
                    Allow Review
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${previewTemplate.settings.proctoring ? 'bg-green-500' : 'bg-gray-300'}`}
                    />
                    Proctoring
                  </div>
                </div>
