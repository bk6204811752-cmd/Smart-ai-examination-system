...
export default function AdminDashboard() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      await Promise.allSettled([loadStats(), loadLiveUsers()])
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      await Promise.all([loadStats(), loadLiveUsers()])
    } catch (error) {
          ...
          <div className="grid lg:grid-cols-3 gap-6 mb-8">
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <button className="w-full p-3 bg-gradient-to-r from-green-50 to-green-100 rounded-lg hover:from-green-100 hover:to-green-200 transition text-left group">
                  <div className="flex items-center space-x-3">
                ...
                <button className="w-full p-3 bg-gradient-to-r from-yellow-50 to-yellow-100 rounded-lg hover:from-yellow-100 hover:to-yellow-200 transition text-left group">
                  <div className="flex items-center space-x-3">
                ...
                <div className="h-64 flex items-end justify-between space-x-1">
                  {analyticsData.slice(-14).map((data, idx) => {
                    const maxValue = Math.max(
                      ...analyticsData.map(d => Math.max(d.users, d.exams * 5, d.violations * 20))
                    )
                    return (
                      <div key={idx} className="flex-1 flex flex-col items-center space-y-1">
                        ...
                        <span className="text-[10px] text-gray-500 rotate-45 mt-2">
                          {data.date}
                        </span>
                      </div>
                    )
                ...
                <div className="space-y-3">
                  {filteredUsers.map(user => (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        {user.role === 'student' && (
                          <span className="text-sm text-gray-600">{user.examsCompleted} exams</span>
                        )}
                        {user.role === 'teacher' && (
                          <span className="text-sm text-gray-600">{user.examsCreated} created</span>
                        )}
                        <button
