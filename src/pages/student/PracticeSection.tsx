...
export default function PracticeSectionPage() {
  ...
  const [activeTab, setActiveTab] = useState<'categories' | 'mock-tests' | 'statistics'>(
    'categories'
  )
            ...
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div>
                ...
                <p className="text-sm sm:text-base text-blue-100">
                  Master every subject with AI-proctored mock tests
                </p>
              </div>
            </div>
            <Link
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {filteredMockTests.map(test => {
                ...
                return (
                  <div
                    key={test.id}
                    className="bg-white rounded-xl shadow-sm border-2 border-gray-100 hover:border-blue-200 hover:shadow-md transition p-5 sm:p-6"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${category?.gradient} rounded-lg flex items-center justify-center`}
                      >
                        <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                      </div>
                      <span
                        className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold ${getDifficultyColor(test.difficulty)}`}
                      >
                        {test.difficulty}
                      </span>
                    </div>
                    ...
                    {test.attempted > 0 && (
                      <div className="mb-4 p-3 bg-green-50 rounded-lg">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Attempted {test.attempted} time(s)
                        </p>
                      </div>
                    )}
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                {categories.map(category => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${category.gradient} rounded-lg flex items-center justify-center flex-shrink-0`}
                      >
                        <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                      </div>
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 text-xs sm:text-sm text-gray-600">
                          <span>
                            0/{mockTests.filter(t => t.category === category.id).length} tests
                          </span>
                          <span>•</span>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-6 sm:p-8 text-center">
              ...
              <p className="text-sm sm:text-base text-gray-600 mb-6">
                Take your first mock test to see your statistics here
              </p>
              <button
