...
export default function GamificationPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'achievements' | 'badges' | 'leaderboard'
  >('overview')
              ...
              <div className="space-y-3">
                {achievements
                  ...
                  .map(achievement => (
                    <div
                      key={achievement.id}
                      className="flex items-center gap-4 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200"
                    >
                      <div
                        className={`bg-gradient-to-br ${getRarityColor(achievement.rarity)} text-white p-3 rounded-lg`}
                      >
                  ...
                  .map(achievement => (
                    <div key={achievement.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-start gap-3 mb-3">
                        <div
                          className={`bg-gradient-to-br ${getRarityColor(achievement.rarity)} opacity-50 text-white p-2 rounded-lg`}
                        >
                          {getAchievementIcon(achievement.icon)}
                        </div>
                        <div className="flex-1">
            ...
            {achievements.map(achievement => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`bg-gradient-to-br ${getRarityColor(achievement.rarity)} ${!achievement.unlocked && 'opacity-50'} text-white p-4 rounded-xl`}
                  >
                    {getAchievementIcon(achievement.icon)}
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <span
                      className={`text-xs px-2 py-1 rounded-full font-bold ${getCategoryColor(achievement.category)}`}
                    >
              ...
              {leaderboard.map(entry => (
                ...
                >
                  ...
                  <div className="text-center min-w-[60px]">
                    ...
                    {entry.trend !== 'same' && (
                      <div
                        className={`text-xs mt-1 ${entry.trend === 'up' ? 'text-green-600' : 'text-red-600'}`}
                      >
