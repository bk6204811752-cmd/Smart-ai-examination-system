...
export default function StudentDashboard() {
  ...
  // Computed stats
  const avgScore =
    results.length > 0 ? results.reduce((sum, r) => sum + r.percentage, 0) / results.length : 0
  const passRate =
    results.length > 0 ? (results.filter(r => r.percentage >= 60).length / results.length) * 100 : 0
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: Award,
      iconColor: avgScore >= 80 ? 'text-green-600' : 'text-amber-600',
    ...
    {
      label: 'Integrity',
      value: totalViolations === 0 ? '100' : Math.max(0, 100 - totalViolations * 10).toString(),
      suffix: '%',
      ...
      icon: Shield,
      iconColor: totalViolations === 0 ? 'text-emerald-600' : 'text-red-600',
  ...
  const quickActions = [
    {
      ...
      desc: 'Mock exams with AI proctoring',
      icon: Play,
      link: '/practice',
      from: 'from-blue-500',
      to: 'to-cyan-500',
  ...
  const badges = [
    ...
    },
    {
      ...
      title: 'Clean Slate',
      desc: 'No violations',
      earned: totalViolations === 0 && results.length > 0,
      color: 'green',
    },
  ]
          ...
          {statCards.map((card, i) => (
            ...
            >
              ...
              <div className="absolute bottom-2 right-2 opacity-60">
                <Sparkline
                  data={card.sparkData.length > 1 ? card.sparkData : [0, 50, 100]}
                  color={card.color}
                />
              </div>
            </motion.div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div>
                    ...
                    <p
                      className={`font-bold text-sm ${trend === 'up' ? 'text-green-600' : trend === 'down' ? 'text-red-600' : 'text-gray-500'}`}
                    >
                      {trend === 'up'
                        ? '📈 Improving'
                        : trend === 'down'
              ...
              <div className="mt-4 space-y-2">
                {[
                  ...
                  {
                    label: 'Integrity',
                    value: Math.max(0, 100 - totalViolations * 10),
                    color: '#8b5cf6',
                  },
                ].map(item => (
                  <div key={item.label}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {quickActions.map((action, i) => (
              ...
              >
                ...
                >
                  <div
                    className={`w-11 h-11 bg-gradient-to-br ${action.from} ${action.to} rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}
                  >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-900 text-sm truncate">
                      {result.exam_title || `Exam ${result.exam_id?.slice(-6)}`}
                    </p>
                    <p className="text-xs text-gray-400">
                      {result.score || 0}/{result.total_points || result.total || 100} marks
                    </p>
                  </div>
